...
| 撮影機材 | カメラ（何台でも）、ライト、反射面（水面・床鏡・立て鏡）、背景布（色自由） |
| 取り込み | GLB / glTF / FBX。ドラッグ&ドロップ対応（フォルダごと可）。単位の自動判定と横倒し補正つき |
| ビュー | 上面 / 側面 / パース / ファインダー。カメラ窓は移動・サイズ変更・開閉ができる |
| 寸法 | 設定はそれぞれの持ち主に。スタジオの寸法はスタジオ、壁・被写体までの距離はカメラごと、大きさは各オブジェクト。定規ボタンは全部まとめての開閉 |
| カメラ | センサーはフォーマット選択（スーパー35・フルサイズ・MFT・手動）、レンズ、比率（既定は 16:9）、高さ、チルト。向きは 0° で奥の壁を向く。複数台置いて、選んだ 1 台の絵を小窓に出す |
| 共有 | 配置を URL に圧縮して埋め込む。サーバー不要。図面とカメラ絵を PNG 書き出し（画面の 2 倍の解像度、寸法の数値も焼き込む） |
| 画面 | 左下の ＋ で置くものを選び、歯車でパネルを開閉する。パネルは リスト / 共有 の 2 タブ。スタジオもリストの 1 行で、選んだものの設定はリストの下に出る。Delete キーで選択中のものを削除 |
...
- カメラ窓の位置とサイズ、設定パネルの開閉、定規の入切、保存したセンサー幅は localStorage に入る。
  どの寸法を出すかは配置の一部なので、共有リンクに乗る。
- 寸法線は 1px の線ではなく、ワールド座標の細い帯（板）で描く。WebGL の線幅は
  ブラウザが 1px に固定するので、拡大や印刷で消えてしまうため。太さは部屋の
  大きさに比例させ、どのスタジオでも同じ太さに見えるようにしている。
- 寸法の数値は画面では HTML なので、そのままでは書き出した画像に写らない。
  PNG 書き出しでは同じ計算で位置を出して 2D で焼き込んでいる。

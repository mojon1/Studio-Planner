...
| ビュー | 上面 / 側面 / パース / ファインダー。カメラ窓は移動とサイズ変更ができる |
| カメラ | センサーはフォーマット選択（スーパー35・フルサイズ・MFT・手動）、レンズ、比率、高さ、チルト。複数台置いて、選んだ 1 台の絵を小窓に出す |
| 共有 | 配置を URL に圧縮して埋め込む。サーバー不要。図面とカメラ絵を PNG 書き出し |
| 画面 | 左下の ＋ で置くものを選び、歯車でパネルを開閉する。パネルは リスト / 共有 の 2 タブ。スタジオもリストの 1 行で、選んだものの設定はリストの下に出る |

...
| ビュー | 上面 / 側面 / 正面 / パース / カメラ全画面。カメラ窓は移動とサイズ変更ができ、**別ウィンドウに切り離してサブモニタへ出せる**。グリッドと寸法は個別に消せる |
...
| カット | 1 つの案件に複数カット。「カット」タブで切り替え・複製・並べ替え。カット名とメモもここ。PDF は「このカットだけ」「全カット」を選べる |
...
| PDF | A4 一枚に上面・側面・パース・ファインダーの 4 面とカメラ情報。案件名・カット・出力日時・メモ入り。A4 横／縦を選択。全カットならカットごとに 1 枚 |
...
「カット」タブでカット名とメモを、「共有」タブで案件名を入れ、用紙の向きを選んで「PDF保存」。
